...
const DEFAULTS = {
  colors: ["#FFFFFF", "#FF8C35", "#FFF200", "#F5EEA5"],
  colorShift: 1,
  opacity: 50,
  thickness: 2,
  trails: 60,
...
const inGamut = (rgb: readonly number[]) => rgb.every((c) => c >= -0.001 && c <= 1.001);

function strokeFor(color: string, maxL: number, alpha: number): string {
  const [r, g, b] = parseColor(color);
  const [L, A, B] = srgbToOklab(r, g, b);
  if (L <= maxL) {
    const rgb2 = [r, g, b].map((c) => Math.round(c * 255));
    return `rgba(${rgb2[0]}, ${rgb2[1]}, ${rgb2[2]}, ${alpha})`;
  }
  ...
  const rgb2 = oklabToSrgb(maxL, cos * fitted, sin * fitted).map((c) =>
...
export default function LineCursor(props: LineCursorProps) {
  const {
